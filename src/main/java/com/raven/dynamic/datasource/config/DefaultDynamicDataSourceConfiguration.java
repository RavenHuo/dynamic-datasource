...
public class DefaultDynamicDataSourceConfiguration extends DynamicDataSourceProvider {
    ...
    @Bean(name = DynamicDataSourceConstant.PRIMARY_DATASOURCE_BEAN_NAME)
    @Qualifier(DynamicDataSourceConstant.PRIMARY_DATASOURCE_BEAN_NAME)
    ...
    @Primary
    public DynamicDataSource dynamicDataSource(@Qualifier(DynamicDataSourceConstant.PRIMARY_DATASOURCE_BEAN_NAME) DataSource primaryDatasource) throws SQLException {
        log.info("default   dynamicDataSource loading--------------------");
        DynamicDataSource dynamicDataSource = new DynamicDataSource();
        Map<String, DataSource> defaultDataSource = new ConcurrentHashMap<>(8);
        defaultDataSource.put(DynamicDataSourceConstant.DYNAMIC_DATASOURCE_DEFAULT_DATASOURCE_NAME, primaryDatasource);
